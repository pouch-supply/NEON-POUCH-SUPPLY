...
import { 
  ...
  Camera, QrCode, UserCheck, Smartphone, Upload, Activity, Check, X, ExternalLink
} from 'lucide-react';
...
export default function CheckoutView({
  ...
  // ✅ Worldpay Access state
  const [isProcessing, setIsProcessing] = useState(false);
  const [paymentError, setPaymentError] = useState<string | null>(null);
  const [paymentSuccessData, setPaymentSuccessData] = useState<any | null>(null);
  const [isWorldpayLoaded, setIsWorldpayLoaded] = useState(false);
  const [worldpaySessionId, setWorldpaySessionId] = useState<string | null>(null);

  // ✅ Check if Worldpay Access Checkout is configured
  useEffect(() => {
    // Check Worldpay config
    fetch('/api/worldpay/config')
      .then(res => res.json())
      .then(data => {
        setIsWorldpayLoaded(data.isConfigured);
        if (!data.isConfigured) {
          console.warn('Worldpay Access is not configured. Please check environment variables.');
        }
      })
      .catch(err => {
        console.error('Failed to fetch Worldpay config:', err);
        setIsWorldpayLoaded(false);
      });
  }, []);
  ...
  useEffect(() => {
    if (!currentDiscount && cartItems.some(i => 
      i.productId?.includes('sub-pack') || 
      i.productTitle?.toLowerCase().includes('subscription') || 
      i.productTitle?.toLowerCase().includes('pack')
    )) {
      ...
      setCurrentDiscount(subDiscount);
      if (onApplyDiscount) {
        onApplyDiscount(subDiscount);
      }
    }
  ...
  const deliveryCost = currentDiscount?.type === 'Free shipping' ? 0 : (subtotalAfterDiscount >= 40 ? 0 : 2.99);
  ...
  // ✅ Handle Worldpay Access Checkout
  const handlePay = async (e: React.FormEvent) => {
    ...
    // If total is 0 (store credit covers everything)
    ...
    // ✅ Check if Worldpay is configured
    if (!isWorldpayLoaded) {
      setPaymentError('Worldpay payment gateway is not configured. Please contact support.');
      return;
    }
    ...
    try {
      ...
      const generatedOrderId = `PS${Date.now()}-${Math.floor(Math.random() * 10000)}`;
      
      // ✅ Create Worldpay Access Session
      ...
      if (!sessionRes.ok) {
        // ✅ Display proper error from Worldpay
        const errorMessage = sessionData.error || sessionData.message || 'Failed to create payment session';
        console.error('Worldpay session error:', sessionData);
      ...
      setWorldpaySessionId(sessionData.sessionId);

      // ✅ Redirect to Worldpay Access Checkout
      if (sessionData.redirectUrl) {
        // Store order info in session for callback handling
        sessionStorage.setItem('pendingOrderId', generatedOrderId);
        ...
        throw new Error('No redirect URL received from Worldpay');
      }
    } catch (error: any) {
      console.error('Worldpay checkout error:', error);
      setPaymentError(error.message || 'Payment initialization failed. Please try again.');
      setIsProcessing(false);
    }
  };

  // ✅ Check for Worldpay callback on mount
  useEffect(() => {
    ...
    // ✅ If returning from Worldpay callback
    if (orderId && status) {
  ...
  const verifyPaymentStatus = async (orderId: string) => {
    ...
    try {
      const statusRes = await fetch(`/api/worldpay/status?orderId=${encodeURIComponent(orderId)}`);
      ...
      if (statusData.paid) {
        ...
        if (pendingData) {
          try {
            ...
            console.warn('Failed to parse pending order data');
          }
        }
        ...
        const successData = {
          ...
          address: orderData.address || `${addressLine}, ${city}, ${postcode}, ${country}`,
          total: orderData.total || finalTotalToPay,
          discountApplied: orderData.discountApplied || currentDiscount,
          ...
          gatewayTxId: statusData.transactionId || `WP-${orderId}`,
          gatewayAuthCode: statusData.authCode || 'AUTH-OK',
          cardBrand: 'Card',
          storeCreditApplied: orderData.storeCreditApplied || storeCreditApplied
        };
        ...
        <div className="bg-white border border-slate-200/80 rounded-3xl p-8 sm:p-12 shadow-2xl text-center space-y-6 relative overflow-hidden">
          ...
          <div className="space-y-2">
            <span className="text-[10px] bg-emerald-100 text-emerald-800 font-extrabold py-1 px-3.5 rounded-full uppercase tracking-wider inline-block">
              ✓ Payment Authorized by Worldpay
            </span>
          ...
          <div className="bg-slate-50 border border-slate-150/70 rounded-2xl p-6 text-left max-w-lg mx-auto space-y-4">
            <div className="flex justify-between border-b border-slate-200 pb-3 text-xs">
              ...
              <span className="font-mono font-bold text-slate-800">{paymentSuccessData.orderId}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-3 text-xs">
              <span className="text-slate-400 font-bold">Transaction ID:</span>
              <span className="font-mono font-bold text-slate-800 uppercase">{paymentSuccessData.gatewayTxId}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-3 text-xs">
              <span className="text-slate-400 font-bold">Auth Code:</span>
              <span className="font-mono font-bold text-slate-800">{paymentSuccessData.gatewayAuthCode}</span>
            </div>
            <div className="flex justify-between border-b border-slate-200 pb-3 text-xs">
              <span className="text-slate-400 font-bold">Payment Method:</span>
              <span className="font-semibold text-slate-800">{paymentSuccessData.cardBrand}</span>
            </div>
            <div className="flex justify-between pt-1 text-sm font-black">
              <span className="text-slate-800 uppercase">Amount Paid:</span>
              <span className="text-slate-950">£{paymentSuccessData.total.toFixed(2)}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-4 max-w-md mx-auto">
            <button
              ...
              className="w-full bg-slate-900 hover:bg-slate-800 text-white py-3.5 px-6 rounded-xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer shadow-md"
            >
              Continue Shopping
            </button>
            <button
              onClick={() => onNavigate('frontend-account')}
              className="w-full bg-white border border-slate-200 hover:bg-slate-50 text-slate-700 py-3.5 px-6 rounded-xl text-xs font-black uppercase tracking-widest transition-all cursor-pointer"
            >
              View Orders
            </button>
          </div>
  ...
  // ✅ Check for subscription account requirement
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-8 md:p-12 shadow-sm space-y-6 flex flex-col items-center animate-fade-in">
          ...
          <div className="space-y-3">
            ...
            <p className="text-slate-500 text-xs leading-relaxed max-w-md mx-auto">
              Your cart contains a subscription pack. Please create an account or sign in to complete your subscription.
            </p>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 w-full max-w-md pt-2">
            ...
            <button
              ...
              className="flex-1 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs uppercase tracking-widest py-3.5 px-6 rounded-xl transition-all border border-slate-200"
            >
              Continue Shopping
            </button>
          </div>
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8 flex items-center justify-between">
        ...
        <div className="flex items-center gap-1.5 text-xs text-slate-400 font-bold">
          <ShieldCheck className="h-4 w-4 text-emerald-500" /> Worldpay Secure Checkout
        </div>
      </div>
        ...
        <div className="lg:col-span-7 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
            <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-500 flex items-center gap-2">
              <Truck className="h-4 w-4 text-indigo-600" /> 1. Shipping Details
            </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5">
                <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 block">Email *</label>
                <input
                  type="email"
                  required
            ...
            <div className="space-y-1.5">
              <label className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 block">Address *</label>
              <input
                type="text"
                required
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-100">
              <h3 className="text-xs font-extrabold uppercase tracking-widest text-slate-500 flex items-center gap-2">
                <CreditCard className="h-4 w-4 text-indigo-600" /> 2. Payment
              </h3>
              {isWorldpayLoaded && (
                <span className="text-[10px] font-extrabold text-emerald-600 flex items-center gap-1">
                  <ShieldCheck className="h-3.5 w-3.5" /> Worldpay Secure
                </span>
              )}
            </div>

            {/* Worldpay Checkout Info */}
            <div className="border border-indigo-100 bg-indigo-50/40 rounded-2xl p-6 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h4 className="text-xs font-black uppercase tracking-wider text-slate-850">
                  {isWorldpayLoaded ? 'Worldpay Secure Checkout' : 'Payment Gateway Not Configured'}
                </h4>
                <p className="text-[11px] text-slate-500 leading-relaxed max-w-md mx-auto">
                  {isWorldpayLoaded 
                    ? 'You will be redirected to Worldpay\'s secure payment page to complete your transaction.'
                    : 'Please contact support to configure the payment gateway.'}
                </p>
              </div>
            ...
            <form onSubmit={handlePay} className="space-y-4">
              {paymentError && (
                <div className="flex gap-2 items-center bg-red-50 border border-red-200 p-3.5 rounded-xl text-xs font-bold text-red-700">
              ...
              <button
                type="submit"
                disabled={isProcessing || !isWorldpayLoaded}
              ...
              >
                {isProcessing ? (
                  ...
                  <>
                    ...
                    <span>Pay £{finalTotalToPay.toFixed(2)} with Worldpay</span>
                  </>
                )}
              </button>
              ...
              {!isWorldpayLoaded && (
                <p className="text-[10px] text-amber-600 text-center font-bold">
                  ⚠️ Worldpay is not configured. Please check environment variables.
                </p>
              )}
            </form>
          </div>
        ...
        <div className="lg:col-span-5 space-y-6">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4 sticky top-6">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <label htmlFor="apply-store-credit-checkout" className="font-extrabold text-[#071d37] cursor-pointer select-none">
                      Apply Store Credit (£{loggedInCustomer.storeCredit.toFixed(2)})
                    </label>
                  </div>
                </div>
                {applyStoreCredit && (
                  <p className="text-[10.5px] text-emerald-700 mt-2 font-medium">
                    Applying £{Math.min(loggedInCustomer.storeCredit, finalTotal).toFixed(2)} credit
                  </p>
                )}
              </div>
            )}
              ...
              {currentDiscount && (
                <div className="flex justify-between text-emerald-600">
                  <span className="flex items-center gap-1">
                    <Check className="h-3 w-3" /> {currentDiscount.title}
                  </span>
            ...
            <div className="border-t border-slate-100 pt-3.5 flex justify-center gap-4 text-slate-400 font-bold text-[9px] uppercase tracking-wider">
              <span className="flex items-center gap-1"><Lock className="h-3 w-3 text-emerald-500" /> SSL</span>
              <span className="flex items-center gap-1"><ShieldCheck className="h-3 w-3 text-indigo-500" /> Worldpay</span>
            </div>
          </div>
